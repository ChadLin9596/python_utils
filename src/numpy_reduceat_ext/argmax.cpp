...
template <typename T>
static inline npy_intp argmax_segment(const T* a, npy_intp beg, npy_intp end) {
...
template <typename T>
static void argmax_reduceat_core(const T* a,
                                 const npy_intp* idxp,
                                 npy_intp idx_len,
                                 npy_intp arr_len,
                                 npy_intp* outp)
{
    for (npy_intp k = 0; k < idx_len; ++k) {
        ...
        outp[k] = (end <= beg) ? beg : argmax_segment<T>(a, beg, end);
    }
}
                            ...
                            npy_intp* outp)
{
    ...
    argmax_reduceat_core<T>(a, idxp, idx_len, arr_len, outp);
}

static PyObject* argmax_reduceat_impl(PyObject* a_obj, PyObject* idx_obj) {
    ...
    // Pointers
    const void* a_data = PyArray_DATA(arr);
    ...
    switch (typenum) {
        ...
        default:
            ...
            PyErr_SetString(PyExc_TypeError, "Unsupported dtype for argmax_reduceat");
            return NULL;
    }
...
// ---- Python glue ----
static PyObject* py_argmax_reduceat(PyObject* self, PyObject* args) {
    PyObject *a_obj, *idx_obj;
    if (!PyArg_ParseTuple(args, "OO", &a_obj, &idx_obj)) return NULL;
    return argmax_reduceat_impl(a_obj, idx_obj);
}

// Method table
static PyMethodDef Methods[] = {
    {"reduceat", (PyCFunction)py_argmax_reduceat, METH_VARARGS,
     "Argmax per ufunc.reduceat semantics (global indices)."},
    {NULL, NULL, 0, NULL}
};
...
static struct PyModuleDef moduledef = {
    ...
    "argmax",           // module name
    "argmax reduceat",  // module doc
    -1,
    Methods
};

// PyInit_<module_name>
PyMODINIT_FUNC PyInit_argmax(void) {
    import_array();
